...
object DNAProfiler {
  ...
  def consensus(profile: List[String]): String = {
    ...
    for (i <- List.range(0, si)) {
      val bases = {
        for (sequence <- profile) yield sequence.substring(i, i + 1)
      }.toList
  ...
  def greedyMotifSearchWithPseudoCounts(dna: List[String], k: Int, t: Int): List[String] = {
    ...
    for (motif1 <- dna(0).sliding(k)) {
      ...
      val score = {
        for (m <- arrMotifs) yield Hamming.calculate(m, consArr)
      }.sum

      val consBest = DNAProfiler.consensus(bestMotifs.toList)
      val bestScore = {
        for (m <- bestMotifs) yield Hamming.calculate(m, consBest)
      }.sum
  ...
  def formMotifs(profile: Array[Array[Double]], dna: List[String]): List[String] = {
    val size = profile(0).size

    val sequences = for (d <- dna)
    yield DNAProfiler.findMostProbable(d, profile)

    return sequences.flatten.toList
  ...
  def randomizedSearch(dna: List[String], k: Int, t: Int, limit: Int): List[String] = {
    ...
    for (step <- List.range(0, limit)) {

      for (d <- dna; i = (Random.nextInt(upperLimitForKmer))) {
        arrMotifs(dna.indexOf(d)) = d.substring(i, i + k)
      }

      //      println(bestMotifs.mkString(" "))
      stop = false

      while (!stop) {

        val profile = DNAProfiler.formProfileWithPseudoCount(arrMotifs.toList)
        val motifs = DNAProfiler.formMotifs(profile.transpose, dna)

        for (i <- List.range(0, t)) arrMotifs(i) = motifs(i)

        val consArr = DNAProfiler.consensus(motifs)
        val score = {
          for (m <- motifs) yield Hamming.calculate(m, consArr)
        }.sum
        ...
        //        println(consArr.mkString + " " + score)
        //        println(consBest.mkString + " " + bestScore)
        //        println("---")

        if (score < bestScore) {
          for (i <- List.range(0, t)) bestMotifs(i) = motifs(i)
        } else {
          println("could not improve at iteration " + step)
          stop = true
        }
      }
    }
  ...
  def generateFibo(probabilities: List[Double]): List[Double] = {
    var acc: Double = 0
    var probs = Array.ofDim[Double](probabilities.size)

    for (i <- List.range(0, probabilities.size)) {
      acc += probabilities(i)
      probs(i) = acc
    }
  ...
  def gibbsSampler(dna: List[String], k: Int, t: Int, numStarts: Int, limit: Int): List[String] = {
    ...
    for (i <- List.range(0, numStarts)) {
      for (d <- dna; i = (Random.nextInt(upperLimitForKmer))) {
        arrMotifs(dna.indexOf(d)) = d.substring(i, i + k)
      }

      for (step <- List.range(0, limit)) {
        val toRemove = Random.nextInt(t)

        val exceptI = {
          for (i <- List.range(0, t); if (i != toRemove)) yield arrMotifs(i)
        }.toList
        val profile = DNAProfiler.formProfileWithPseudoCount(exceptI)
        //        println("----")
        //        for (p<-profile) println(p.mkString(" "))
        //        println("----")
        val motifs = DNAProfiler.generate(k, dna(toRemove))
        //  DNAProfiler.formMotifs(profile.transpose,List(dna(toRemove)))
        //        val motifs = DNAProfiler.formMotifs(profile.transpose,dna)

        val probabilities = {
          for (motif <- motifs) yield DNAProfiler.score(motif, profile.transpose)
        }.toList
        val toSelect = Random.nextDouble() * probabilities.max
        //        println(probabilities.mkString(" "))
        //        println(motifs.mkString(" "))

        val newMotif = motifs.zip(probabilities).toList.filter(_._2 >= toSelect).sorted.reverse.head
        arrMotifs(toRemove) = newMotif._1
...
//        println("---")
//        println("c: " + consArr.mkString + " s: " + score + " arr: " + arrMotifs.mkString(","))
//        println("c: " + consBest.mkString + " s: " + bestScore + " bst: " + bestMotifs.mkString(","))
//        println("---")
